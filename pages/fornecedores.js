...
export default function Fornecedores() {
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    setError('');
    const { error: insertError } = await supabase.from('fornecedores').insert([form]);
    setSaving(false);
    if (insertError) {
      ...
      <div className="wide-page-inner">
        ...
        {canEdit && (
          <form className="section-card" onSubmit={handleSubmit} style={{ marginBottom: 24 }}>
            <h2 style={{ marginBottom: 14 }}>Novo fornecedor</h2>
        ...
        <div className="data-table-wrap">
          {items.length === 0 ? (
            <p className="empty-hint">Nenhum fornecedor cadastrado ainda.</p>
          ) : (
