...
// Cobranças cujo aviso de "nova cobrança cadastrada" ainda não foi marcado como visto.
export function getCobrancasNovasNaoLidas(cobrancas, mapaNomes) {
  return (cobrancas || [])
    .filter((item) => !item.aviso_lido)
    .map((item) => ({
      id: item.id,
      texto: `${mapaNomes[item.created_by] || 'Usuário'} cadastrou uma cobrança no projeto ${nomeProjeto(item)} com ${nomeFornecedorOuCliente(item)}, em ${item.created_at ? new Date(item.created_at).toLocaleString('pt-BR') : '—'}.`,
    }));
}
