...
export function generateClientePdfBlob(cliente, filhos, projetosVinculados, projetosList) {
  const doc = new jsPDF();
  ...
  const vinculos = (projetosVinculados || []).filter((p) => p.projeto_id);
  if (vinculos.length === 0) {
    campo('Projetos', 'Nenhum');
  } else {
    vinculos.forEach((p, index) => {
      const nomeProjeto = projetosList.find((proj) => proj.id === p.projeto_id)?.nome;
      campo(`Projeto ${index + 1}`, nomeProjeto);
      campo('Endereço da obra - CEP', p.cep_obra);
      campo('Endereço da obra - Logradouro', p.logradouro_obra);
      campo('Endereço da obra - Número', p.numero_obra);
      campo('Endereço da obra - Complemento', p.complemento_obra);
      campo('Endereço da obra - Bairro', p.bairro_obra);
      campo('Endereço da obra - Cidade', p.cidade_obra);
      campo('Endereço da obra - UF', p.uf_obra);
    });
  }
