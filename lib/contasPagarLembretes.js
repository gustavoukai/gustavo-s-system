...
// Contas a pagar cujo vencimento já passou (até 5 dias) e o status ainda não é "pago".
export function getContasAtrasadas(contas, hoje = new Date()) {
  const hojeSemHora = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate());
  ...
  (contas || []).forEach((conta) => {
    ...
    const vencimento = new Date(conta.ano, conta.mes - 1, conta.dia_vencimento);
    const diffDias = Math.round((hojeSemHora - vencimento) / (1000 * 60 * 60 * 24));
...
export function mensagemAtraso(diffDias) {
  return `venceu há ${diffDias} dia${diffDias > 1 ? 's' : ''}`;
}
