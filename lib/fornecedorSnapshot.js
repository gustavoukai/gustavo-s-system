...
export function generateFornecedorPdfBlob(fornecedor, statusLabel, projetosTrabalhados) {
  const doc = new jsPDF();
  ...
  titulo('Trabalhou em');
  const listaProjetos = projetosTrabalhados || [];
  if (listaProjetos.length === 0) {
    campo('Projetos', 'Nenhum');
  } else {
    listaProjetos.forEach((p, index) => {
