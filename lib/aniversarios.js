...
// Monta a lista de lembretes de aniversário (cliente, cônjuge e filhos) para os próximos 7 dias.
...
function diasDesdeUltimoAniversario(dataNascimento, hoje) {
  if (!dataNascimento) return null;
  const nascimento = new Date(dataNascimento + 'T00:00:00');
  if (isNaN(nascimento.getTime())) return null;

  const hojeSemHora = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate());
  let ultimo = new Date(hoje.getFullYear(), nascimento.getMonth(), nascimento.getDate());
  if (ultimo > hojeSemHora) {
    ultimo = new Date(hoje.getFullYear() - 1, nascimento.getMonth(), nascimento.getDate());
  }

  const diffMs = hojeSemHora - ultimo;
  return Math.round(diffMs / (1000 * 60 * 60 * 24));
}

// Aniversários que já passaram, até 5 dias atrás.
export function getAniversariosPassados(clientes, hoje = new Date()) {
  const lembretes = [];
  ...
  (clientes || []).forEach((cliente) => {
    ...
    candidatos.forEach((c) => {
      if (!c.nome || !c.data) return;
      const dias = diasDesdeUltimoAniversario(c.data, hoje);
      if (dias !== null && dias >= 1 && dias <= 5) {
        lembretes.push({
          texto: `Aniversário de ${c.nome} (${c.papel}) foi há ${dias} dia${dias > 1 ? 's' : ''}.`,
          dias,
        });
      }
    });
